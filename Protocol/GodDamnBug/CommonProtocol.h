...
enum en_PACKET_TYPE
{
	...
	// 이로 인해서 새로 발급 받은 세션키가 틀리다고 나올 수 있기 때문에 Parameter 를 이용한 추가 데이터로 검증을 한 번 더 한다?
	//
	//	{
	//		WORD	Type
	//
...
// 클라이언트가 로그인 서버에 로그인 했을 때 응답 define
enum en_PACKET_CS_LOGIN_RES_LOGIN 
{
	...
	dfLOGIN_STATUS_SESSION_MISS		= 4,		// SessionKey 테이블에 AccountNo 없음
	dfLOGIN_STATUS_STATUS_MISS		= 5,		// Status 테이블에 AccountNo 없음


	// 게임 서버 또는 채팅 서버가 아직 로그인 서버에 로그인하지 않은 상태
	dfLOGIN_STATUS_NOSERVER			= 6,		// 서비스중인 서버가 없음.
};


// 클라이언트가 게임 서버에 로그인 했을 때 응답 define
...
// 모니터링 클라이언트가 모니터링 서버에 로그인했을 때 응답받는 결과
